...
export default function Admin({ cabins }: { cabins: { id: string; name: string; images: { id: string }[] }[] }) {
  return <Layout><Seo title="Admin | Cabañas Corona" path="/admin"/><main className="mx-auto max-w-[960px] px-5 py-12"><h1 className="text-5xl">Cabin admin</h1><p className="text-muted">Edit bilingual descriptions and manage display photos.</p><div className="mt-8 grid gap-4 sm:grid-cols-3">{cabins.map(c => <Link key={c.id} href={`/admin/cabins/${c.id}`} className="border border-cardborder bg-card p-6 shadow"><h2 className="text-2xl">{c.name}</h2><p className="font-sans text-sm">{c.images.length} photo{c.images.length === 1 ? "" : "s"}</p><span className="font-sans text-xs font-bold uppercase text-terra">Manage →</span></Link>)}</div></main></Layout>;
}
